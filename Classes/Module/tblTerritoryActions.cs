...
{
    public class tblTerritoryActions : IReadable<tblTerritoryDTO>
    {
        public tblTerritoryDTO GET(int ID)
        {
            try
            {
                using (climateEntities entidad = new climateEntities())
                {
                    return Mapper.Map<tblTerritoryDTO>(entidad.tblTerritory.Where(p => p.ID == ID).Select(p => p).FirstOrDefault());
                }
            }
            catch (Exception ex)
            {
        ...
        public List<tblTerritoryDTO> GETALL()
        {
            try
            {
                using (climateEntities entidad = new climateEntities())
                {
                    return Mapper.Map<List<tblTerritoryDTO>>(entidad.tblTerritory.Select(p => p));
                }
            }
            catch (Exception ex)
            {
