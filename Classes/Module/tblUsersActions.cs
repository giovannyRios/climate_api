...
{
    public class tblUsersActions
    {
        public tblUsersDTO GET(string userPassword, string user)
        {
            try
            {
                using (climateEntities entidad = new climateEntities())
                {
                    return Mapper.Map<tblUsersDTO>(entidad.tblUsers
                        .Where(p => p.userPassword.Equals(userPassword) && p.userName.Equals(user))
                        .Select(p => p)
                        .FirstOrDefault());
                }
            }
        ...
        public List<tblUsersDTO> GETALL()
        {
            try
            {
                using (climateEntities entidad = new climateEntities())
                {
                    return Mapper.Map<List<tblUsersDTO>>(entidad.tblUsers.Select(p => p));
                }
            }
            catch (Exception ex)
            {
